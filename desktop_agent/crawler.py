...
# Placeholder for logging bridge. Will be overwritten by agent.py
...
def run_naukri_crawler(page, query, location, max_jobs, settings, scraped_jobs_list):
    ...
    for i, jt in enumerate(job_tuples):
        ...
        try:
            ...
            scraped_count += 1
            add_log("INFO", f"[Crawler] Naukri: Scraped '{title}' at '{company}' (details saved to local upload queue)")
...
def run_linkedin_crawler(page, query, location, max_jobs, settings, scraped_jobs_list):
    ...
    for i, card in enumerate(job_cards):
        ...
        try:
            ...
            scraped_jobs_list.append(job_data)
            scraped_count += 1
            add_log("INFO", f"[Crawler] LinkedIn: Scraped '{title}' at '{company}' (details saved to local upload queue)")
...
def run_job_search():
    ...
    add_log("INFO", "[Crawler] Starting decoupled JobForge AI Search Automation Engine...")
        ...
        if chrome_path and os.path.exists(chrome_path):
            ...
            try:
                browser_context = p.chromium.launch_persistent_context(
                    ...
                    channel="chrome",
                    headless=False, # Must be False to allow captcha resolution
                    slow_mo=1000,
            ...
            add_log("WARNING", f"[Crawler] No valid Chrome profile path found at '{chrome_path}'. Launching a clean private browser session...")
            fallback_mode = True
        ...
        try:
            ...
            for location in locations:
                for query in queries:
                    ...
                    # Naukri search
                    try:
                        n_count = run_naukri_crawler(page, query, location, max_jobs_per_query, settings, scraped_jobs)
                        scraped_total += n_count
                    except Exception as e:
                    ...
                    if not CRAWLER_RUNNING:
                        break
                    try:
                        l_count = run_linkedin_crawler(page, query, location, max_jobs_per_query, settings, scraped_jobs)
                        scraped_total += l_count
                    except Exception as e:
                        add_log("ERROR", f"[Crawler] LinkedIn crawling failed for '{query}': {str(e)}")
                        
            add_log("INFO", f"[Crawler] Local search automation complete. Successfully scraped {scraped_total} job items.")
            
            # Upload raw scraped jobs to Cloud SaaS Server!
            if scraped_total > 0:
                add_log("INFO", f"[Crawler] Batch uploading {scraped_total} raw scraped jobs to Cloud SaaS Server for AI matching...")
                try:
                    req = urllib.request.Request(
                        "http://127.0.0.1:8000/api/jobs/upload",
                        data=json.dumps(scraped_jobs).encode('utf-8'),
                        headers={'Content-Type': 'application/json'}
                    )
                    with urllib.request.urlopen(req, timeout=60) as response:
                        res = json.loads(response.read().decode())
                        add_log("IMPORTANT", f"[Crawler] Cloud Gateway uploaded successfully! Server response: {res.get('message', '')}")
                except Exception as upload_err:
                    add_log("ERROR", f"[Crawler] Failed to batch upload scraped jobs to Cloud: {str(upload_err)}")
            else:
                add_log("WARNING", "[Crawler] Scrape finished with zero results. Nothing to upload.")
