...
# Placeholder for status bridge. Will be overwritten or logged
def update_job_status(job_id, status):
    add_log("INFO", f"[Applier] Local update job status: {job_id} -> {status}")
...
def fill_easy_apply_fields(page, job, settings):
    ...
    for field in text_inputs:
        try:
            ...
            if not val: # Only fill if currently empty
                ...
                # Intelligent autofill values based on profile
    ...
    if file_inputs:
        ...
        parent_resume_path = os.path.join(os.path.dirname(resume_dir), "Rohit_Singh_Resume.pdf")
        
        if os.path.exists(parent_resume_path) and not os.path.exists(resume_path):
            resume_path = parent_resume_path
            
        if not os.path.exists(resume_path):
            with open(resume_path, "w") as f:
                f.write("%PDF-1.4 ... Rohit Singh Resume Placeholder ...")
...
def run_job_application(job):
    ...
    add_log("INFO", f"[Applier] Starting decoupled application automation for '{job.get('title', 'Untitled')}' at '{job.get('company', 'Unknown')}'...")
    ...
    with sync_playwright() as p:
        ...
        if chrome_path and os.path.exists(chrome_path):
            add_log("INFO", f"[Applier] Attempting to hook Chrome profile: {chrome_path}...")
            try:
                browser_context = p.chromium.launch_persistent_context(
                    user_data_dir=chrome_path,
                    channel="chrome",
                    headless=False, # Must be visible for Easy Apply forms & review
                    slow_mo=1200,
        ...
        try:
            ...
            if success:
                add_log("INFO", f"[Applier] Application process completed successfully.")
            else:
                add_log("WARNING", f"[Applier] Could not fully complete application automatically.")
